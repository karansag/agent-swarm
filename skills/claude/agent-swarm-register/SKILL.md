...
description: Use when the user asks this Claude Code agent to register itself with the local agent-swarm communication server, join the agent communication protocol, make itself reachable by other agents, or verify its agent-swarm identity.
user-invocable: true
allowed-tools:
  - Bash
---
...
- Pass your exact model id with `--model` (for example `--model claude-opus-4-7`),
